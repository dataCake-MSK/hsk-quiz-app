...
- **DB가 바뀌는 작업 뒤에는 데이터 모델링 설명을 함께 한다**(학습 목표 ①). 테이블 추가·컬럼 변경·제약 추가·마이그레이션이 있으면 보고에 다음을 포함한다.
  1. **스키마**: `psql "\d <테이블>"` 결과를 그대로 보여준다(컬럼·타입·기본값·인덱스·제약·FK)
  2. **실제 데이터 샘플**: `SELECT ... LIMIT 5~6` 결과를 표로 보여준다(비밀 값·개인 정보 컬럼은 제외하거나 앞 몇 글자만)
  3. **설계 이유**: 왜 이 키·제약·인덱스를 택했는지, 대안은 무엇이었는지 한두 줄
  4. **새로 나온 용어**는 뜻을 풀어 쓰고, 필요하면 `learn` 스킬로 [용어 노트](docs/learning/db-terms.md)에 추가한다
